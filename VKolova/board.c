...
void PrintBoard(struct board_t board) {
	...
	printf("\n\nPLAYER: COMPUTER (%dhp), MANA %d/%d, TURN:%d", board.Player[0].health, board.Player[0].manapool.mana, board.Player[0].manapool.max_mana, board.Player[0].turn);
	...
	printf("\n___________________________________________________________________\n");
	printf("PLAYER: PLAYER (%dhp), MANA %d/%d, TURN:%d\n", board.Player[1].health, board.Player[1].manapool.mana, board.Player[1].manapool.max_mana, board.Player[1].turn);	
}
...
int DeckFromFile(struct board_t *board, int pPlayer, char *filename) {	
	...
	while( fgets(str, 1800, fp) ) {
		...
		buff = strtok(NULL, ",");
		sscanf(buff,"%d", &board->Player[pPlayer].deck.card_deck[board->Player[pPlayer].deck.size].s_attack);
		//END!!
...
int init_board(struct board_t *board) {
	...
	board -> Player[0].turn = 0;
	board -> Player[1].turn = 0;
...
int play_card(struct board_t *board, int first_pl, int card, int num_lane){
	...
	if (result == 1) {
		...
		use_card(card_to_board, &board->Player[first_pl].manapool);

			//PROJECT SPECIAL CARDS STARTS HERE !
		switch (card_to_board.s_attack) {
			case 1: //increasing player's health
					change_hp(&board->Player[first_pl].health, card_to_board.health);
					board -> Open_Spots[board -> Card_Positions[num_lane][first_pl]] = 1;
					board -> Card_Positions[num_lane][first_pl] = -1;
					break;
			case 2: //direct damage to the opponent
					change_hp(&board->Player[other_pl].health, -card_to_board.damage);
					board -> Open_Spots[board -> Card_Positions[num_lane][first_pl]] = 1;
					board -> Card_Positions[num_lane][first_pl] = -1;
					break;
			case 3: //increasing player's mana
					board->Player[first_pl].manapool.mana += card_to_board.health;
					board -> Open_Spots[board -> Card_Positions[num_lane][first_pl]] = 1;
					board -> Card_Positions[num_lane][first_pl] = -1;
					break;
		}
