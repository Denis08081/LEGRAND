...
void PrintBoard(struct board_t board) {
	...
	printf("\n\n\n\n\nPLAYER: PLAYER 1 (%dhp), MANA %d/%d, TURN:x",board.Player[0].health,board.Player[0].manapool.mana ,board.Player[0].manapool.max_mana);
...
int play_card(struct board_t *board, int first_pl, int card, int num_lane){
	...
	if (result == 1) {
		...
		MP_use_card(card_to_board, &board->Player[first_pl].manapool);
			...
			//PROJECT SPELL CARDS STARTS HERE!
